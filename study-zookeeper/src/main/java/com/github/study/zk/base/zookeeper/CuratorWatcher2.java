...
public class CuratorWatcher2 {
    private static final String CONNECT_ADDR = "10.200.6.197:2181";
    ...
    public static void main(String[] args) throws Exception {
        ...
        curator.start();
        //第三个参数表示是否接收节点数据内容
        PathChildrenCache childrenCache = new PathChildrenCache(curator, "/super1", true);
        /**
        ...
        curator.create().forPath("/super1", "123".getBytes());
        curator.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT).forPath("/super1/c1", "c1内容".getBytes());
        //经测试，不会监听到本节点的数据变更，只会监听到指定节点下子节点数据的变更
        curator.setData().forPath("/super1", "456".getBytes());
        curator.setData().forPath("/super1/c1", "c1新内容".getBytes());
        curator.delete().guaranteed().deletingChildrenIfNeeded().forPath("/super1");
        Thread.sleep(5000);
        curator.close();
    }
}
