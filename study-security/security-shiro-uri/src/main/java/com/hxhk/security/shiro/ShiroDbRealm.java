...
public class ShiroDbRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		if (useCaptcha) {
			com.hxhk.security.shiro.CaptchaUsernamePasswordToken token = (com.hxhk.security.shiro.CaptchaUsernamePasswordToken) authcToken;
			
			String parm = token.getCaptcha();
			String c = (String)SecurityUtils.getSubject().getSession()
					.getAttribute(com.hxhk.security.shiro.SimpleCaptchaServlet.CAPTCHA_KEY);
			// 忽略大小写
			if (!parm.equalsIgnoreCase(c)) {
				throw new com.hxhk.security.shiro.IncorrectCaptchaException("验证码错误！");
			} 
		} 
