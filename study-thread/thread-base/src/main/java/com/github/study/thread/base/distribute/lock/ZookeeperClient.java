package com.github.study.thread.base.distribute.lock;
...
/**
 * @author longhairen
 * @create 2017/9/5 0005 上午 11:47
 */
public class ZookeeperClient  implements Watcher {

    private static class PropertiesDynLoading {

        public static final String connectString = "10.112.3.145:2181";
    ...
    public void recursionDelete(String nodeName) {
        List<String> children = getChildren(nodeName);
        if(children != null && children.size() > 0) {
            for (String node : children) {
                recursionDelete(nodeName + "/" + node);
            }
        }else {
            deleteNode(nodeName);
        }
    ...
    public static void main(String[] args) {
        ...
        for (String nodeName : list) {
            if (!"zookeeper".equals(nodeName)){
                zkc.recursionDelete("/" + nodeName);
            }
        }
        list = zkc.getChildren("/");
